...
const TopLinks = ({ location }) => (
  <aside id='top-links'>
    ...
    {_locIsNotHome(location) && (
      <span className='line' />
    )}
    {_locIsNotHome(location) && (
      <Link className='home ball btn' to={`/${t('url__all-games')}`} title={t('back-to-all-games')}>
        <Icon type='house' size='22' />
      </Link>
    )}
    {lang === 'en' ? '' : (
      <a onClick={() => _goToLang('en')} className='flag btn ball flag-uk' title='English Version'>
        <Icon type='flag-uk' size='18' />
      </a>
    )}
    {lang === 'pt-br' ? '' : (
      <a onClick={() => _goToLang('pt-br')} className='flag btn ball flag-br' title='Versão em português (BR)'>
